...
const uploadImageToBlob = (imageFile, onSuccess, onError, onProgress) => {
 ...
 fetch(`${API_BASE_URL}/uploadImage?type=articleimages`, {
    method: 'POST',
    body: formData
})
        .then(response => {
...
function IndependentImageUpload() {
    const [selectedFiles, setSelectedFiles] = useState(new Array(3).fill(null));
...
export default IndependentImageUpload;